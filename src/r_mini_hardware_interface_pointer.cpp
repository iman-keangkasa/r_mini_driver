#include "r_mini_driver/r_mini_hardware_interface_pointer.h"
...
RminiHW::RminiHW (double period, const std::string port_id, const uint32_t baud_rate)
  ...
  counter_(0)
{
  //initialize all the dynamixel workbench and its private member
  writeFirst();
...
RminiHW::~RminiHW() 
{
  ...
  {
    const char* log = NULL;
    result = dxl_wb_->torqueOn(dxl.second, &log);
    
    if ( result == false )
    { 
      ROS_ERROR("Unable to keep torque on for: %s", dxl.first().c_str());
    }
  }
}
*/
...
void RminiHW::writeFirst(void) 
{
  ...
  //MOST IMPORTANT otherwise the dxl_wb_ crash 
  //Initialize dxl_wb_ pointer
